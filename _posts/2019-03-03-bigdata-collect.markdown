...
###### 收集服务器上面是否要落地数据  
  有的架构是收集服务器上面不做数据落地，直接通过flume或者其它插件将数据直接发送给后面的消息队列。不在服务器上面落盘，这种方式的好处是收集服务器本身IO处理较低
，同时问题是出现后面的消息队列故障将直接导致收集的数据丢失。因此需要根据业务的重要性进行选择收集服务器的数据是否落地后，再进行消息传输，还是收集直接发送Kafka消息。
  
###### 如何压缩数据
  通常数据收集上来后，在推送到存储之前我们一般进行数据压缩方面的工作，以将数据空间利用率最大化。通过我们考虑离线方面的引擎通常使用gz的方式进行文本数据
的压缩工作，一般按将小文件进行整合成固定范圍的数据大小，即小文件合并，大文件拆分。

#### 能不能自己造个轮子  
```$xslt
给出一个JAVA版的简单版的类Ngninx HTTP数据收集服务器
